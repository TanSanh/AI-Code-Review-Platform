...
import { Loader2, ArrowRight, ArrowLeft, CheckCircle, Shield, KeyRound } from 'lucide-react';
...
export default function ForgotPasswordPage() {
  ...
  const handleResetPassword = async () => {
    ...
    else if (newPassword.length < 8) e.newPassword = t('auth.passwordHint');
    if (!confirmPassword) e.confirmPassword = t('auth.passwordRequired');
    else if (newPassword !== confirmPassword) e.confirmPassword = t('auth.passwordMismatch');
    if (Object.keys(e).length > 0) { setErrors(e); return; }

    setBusy(true);
    setErrors({});
    try {
      await api.resetPassword(email, otpToken, newPassword);
      setSuccess(true);
    } catch (err) {
      setErrors({ general: err instanceof Error ? err.message : t('auth.registerFailed') });
    } finally {
        ...
        <div className="w-full max-w-md overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl dark:border-[#33355a] dark:bg-[#242640]">
          <div className="p-6 text-center">
            ...
            <Button
              ...
              className="mt-4 h-11 w-full rounded-lg bg-gray-900 text-white font-semibold hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-white"
            >
              {t('auth.goToLogin')}
            </Button>
          </div>
    ...
    <div className="flex flex-col items-center justify-center bg-gray-50 px-4 py-12 dark:bg-[#1a1b2e]">
      ...
      <div className="mb-6 flex flex-col items-center">
        ...
        <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
          {step === 1 ? t('auth.forgotPasswordDesc') : step === 2 ? t('auth.step2Desc') + ' ' + email : t('auth.resetPasswordDesc')}
        </p>
      </div>
        ...
        <div className="p-6">
          ...
          {step === 2 && (
            ...
            <div className="space-y-5">
              ...
              >
                {busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : t('auth.next')}
              </Button>
          ...
          {step === 3 && (
            ...
            <div className="space-y-4">
              ...
              <Button
                ...
                className="h-11 w-full rounded-lg bg-gray-900 text-white font-semibold hover:bg-gray-800 disabled:opacity-50 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-white"
              >
                {busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : t('auth.resetPassword')}
              </Button>

              <button
                onClick={() => { setStep(2); setNewPassword(''); setConfirmPassword(''); setErrors({}); }}
